...
protected:
	...
	static int DEADBANDLOW=482;   //decrease this value if drift occurs, increase it to increase sensitivity around the center position
	static int DEADBANDHIGH=542;  //increase this value if drift occurs, decrease it to increase sensitivity around the center position

	int iPOS_MIN=75;  //fully retracted
	int iPOS_MAX=3200; //fully extended
	...
	static int speed = 5;        // velocidad a la que cambiamos la posición. Para hacerlo fino usar 1
	...
	{
		//initialize servos
		servoX.attach(iPinServo, iPOS_MIN, iPOS_MAX);  // conectamos el servo en este pin y establecemos los valores mínimo y máximo
